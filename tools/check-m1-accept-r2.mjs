#!/usr/bin/env node
// M1 独立验收评审（m1-accept-r2）机械验收器 —— 冻结于基线提交。
// 用法：node tools/check-m1-accept-r2.mjs --baseline <40hex>
...
const REPORT = 'docs/milestones/m1/reviews/acceptance-r2.md';
const failures = [];
...
for (const ev of ['contract-gen', 'jsonschema', 'declared=66']) {
  ok(`r2-evidence[${ev}]`, report.includes(ev), 'missing r2 real-chain evidence mention');
}
// 10. AC-01 外部阻断口径
