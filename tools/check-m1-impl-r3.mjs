#!/usr/bin/env node
// M1 实现迭代 2（m1-impl-r3）机械验收器 —— 针对独立验收 R1 发现 F01-F12 的强化门禁。
// 用法：
//   node tools/check-m1-impl-r3.mjs --mode structural --baseline <40hex> --six <sha> --plan <sha> --dplan <sha>
//   node tools/check-m1-impl-r3.mjs --mode cargo-check
//   node tools/check-m1-impl-r3.mjs --mode cargo-test
//   node tools/check-m1-impl-r3.mjs --mode drift-real
//   node tools/check-m1-impl-r3.mjs --mode selftests-real
...
if (mode === 'structural') {
  ...
  const forbiddenExact = new Set(['tools/check-m1-plan.mjs','tools/check-m1-impl-r1.mjs','tools/check-m1-impl-r3.mjs','tools/check-m1-accept-r1.mjs','docs/milestones/m1/reviews/acceptance-r1.md']);
  ...
  ok('stub-route-probe-13', /route_probe=13\/13/i.test(rs.out), 'self-test must probe all 13 registry routes (route_probe=13/13)');
} else {
