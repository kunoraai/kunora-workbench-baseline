#!/usr/bin/env node
// M1 实现迭代 2（m1-impl-r2）机械验收器 —— 针对独立验收 R1 发现 F01-F12 的强化门禁。
// 用法：
//   node tools/check-m1-impl-r2.mjs --mode structural --baseline <40hex> --six <sha> --plan <sha> --dplan <sha>
//   node tools/check-m1-impl-r2.mjs --mode cargo-check
//   node tools/check-m1-impl-r2.mjs --mode cargo-test
//   node tools/check-m1-impl-r2.mjs --mode drift-real
//   node tools/check-m1-impl-r2.mjs --mode selftests-real
...
if (mode === 'structural') {
  ...
  const forbiddenExact = new Set(['tools/check-m1-plan.mjs','tools/check-m1-impl-r1.mjs','tools/check-m1-impl-r2.mjs','tools/check-m1-accept-r1.mjs','docs/milestones/m1/reviews/acceptance-r1.md']);
  ...
  if (invPath) {
    ...
    writeFileSync(tmp, txt.replace(/\bWUI-[A-Z0-9-]+/, 'WUI-DUPLICATED') + '\nWUI-DUPLICATED\n', 'utf8');
    const bad = runCargo(['run', '--locked', '-p', 'capability-report', '--', '--self-test', '--inventory', tmp], 900 * 1000);
    ok('capreport-inventory-enforced', bad.code !== 0, 'corrupted/duplicated inventory was NOT detected');
    try { unlinkSync(tmp); } catch {}
  }
  // F06/F07: fake/stub 自检必须包含真实网络启动证据
  const fh = runCargo(['run', '--locked', '-p', 'fake-harness', '--', '--self-test'], 900 * 1000);
  ok('fake-harness-loopback', /127\.0\.0\.1:\d+/.test(fh.out) && /ready/i.test(fh.out) && /probe/i.test(fh.out), 'self-test must bind loopback, emit ready URL, and probe');
  const rs = runCargo(['run', '--locked', '-p', 'reference-stub', '--', '--self-test'], 900 * 1000);
  ok('reference-stub-started', /127\.0\.0\.1:\d+/.test(rs.out) && /(registry|client)/i.test(rs.out), 'self-test must start registry server and exercise client');
} else {
