#!/usr/bin/env node
// M2 实现任务（m2-impl-r2）机械验收器 —— 依 M2 详细计划（frozen）构建。
// 模式：
...
if (mode === 'structural') {
  ...
  // M2：fake Harness fixture 自检（ready/cookie/probe/crash/挂起场景；连续 2 次防 Windows loopback 瞬时）
  for (let attempt = 1; attempt <= 2; attempt++) {
    const fh = runCargo(['run', '--locked', '-p', 'fake-harness', '--', '--self-test'], 900 * 1000);
    ok(`fake-m2-fixtures[${attempt}]`, fh.code === 0 && /ready|crash|hang|probe|cookie|token/i.test(fh.out), `run ${attempt} exit=${fh.code} tail=${fh.out.slice(-160)}`);
  }
  // M2：reference stub 本地输入 driver 自检
  for (let attempt = 1; attempt <= 2; attempt++) {
    const rs = runCargo(['run', '--locked', '-p', 'reference-stub', '--', '--self-test'], 900 * 1000);
    ok(`stub-m2-driver[${attempt}]`, rs.code === 0 && /driver|scenario|STALE|FENCED|unreachable/i.test(rs.out), `run ${attempt} exit=${rs.code} tail=${rs.out.slice(-160)}`);
  }
