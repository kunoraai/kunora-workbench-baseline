#!/usr/bin/env node
// M2 实现任务（m2-impl-r5）机械验收器 —— 依 M2 详细计划（frozen）构建。
// 模式：
...
if (mode === 'structural') {
  ...
  // F08 门：M2 文档不得含仓库根相对断链模式
  const m2doc = trackedFilesUnder('docs/milestones/m2/').filter((f) => f.endsWith('.md')).map((f) => readIf(f)).join('\n');
  ok('no-repo-root-links', !/\]\(docs\/(?:milestones\/m2|dshd-service-design|backend-node-hld|contracts|dsh)\//.test(m2doc), 'M2 docs must not contain repo-root-relative broken links (F08)');
  ...
  ok('shutdown-convergence-test', /stopping|converge|shutdown/.test(coreTests), 'shutdown-convergence regression test required (F09)');
  // runner M2 driver
