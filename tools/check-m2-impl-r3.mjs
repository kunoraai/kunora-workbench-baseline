#!/usr/bin/env node
// M2 实现任务（m2-impl-r3）机械验收器 —— 依 M2 详细计划（frozen）构建。
// 模式：
...
if (mode === 'structural') {
  ...
  ok('t14-product-path', /dshd/.test(t14), 't14 must state the evidence comes from the dshd-assembled product path (not diagnostic driver)');
  ...
  ok('ac03-product-path', /AC-03[^\n]*PASS/.test(exitRec), 'AC-03 must be PASS: product-path (dshd-assembled) real N+1 transcript required, diagnostic driver evidence not sufficient');
