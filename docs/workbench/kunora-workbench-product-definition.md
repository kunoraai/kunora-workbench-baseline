...
| v0.9 | 2026-08-29 | Codex | 评审中 | 废止人工启动及启动门禁；任务关联后自动通过中央服务调用 dshd 完成初始化。 | 用户指令 [BIZ-04] |
| v0.10 | 2026-08-29 | Codex | 评审中 | 明确 Kunora 管理对象称为“任务”，dsh 执行单元称为“作业”，并补充初始化失败恢复的工作台要求。 | 用户指令 [BIZ-06] |

# Kunora Workbench 产品简报

2026-08-30 UI/UX 修订说明：关联时任务名称、介绍均可选，完整度不阻止任务创建、初始化或消息交互。任务绑定仓库与分支、执行节点在任务工作面板内只读。成果生成与验收分开建模，验收结论绑定不可变成果版本，新版本需重新验收；以上为设计要求，接口实现状态需另行核对。详细规则见 [修订交互规范](ui-mockups/revision-2026-08-30/interaction-spec.md)。
...
4. 工作台关联仓库分支并创建任务后，自动请求中央服务分配节点；中央服务调用对应 `dshd` 完成任务初始化。[BIZ-04]
5. 中央服务可以主动连接任意一台已注册的 `dshd`，向指定节点透传用户操作指令和消息，并把执行响应传回上层。
6. `dshd` 可以主动向中央服务上报心跳、状态和日志等信息，中央服务存储这些信息并供上层查询。
7. 中央数据库保存任务与后端节点的关系，以及任务相关信息、初始化和执行进展、节点状态和日志等数据。[BIZ-04]
8. 每个后端节点是一个独立 Docker 部署单元，内部运行一个 `dshd` 和一个由它管理的 DeepSeek Harness。
9. `dshd` 是节点唯一对外服务和控制入口；DeepSeek Harness 只监听容器内 loopback，不直接对中央服务或上层暴露。

## 3. 产品特性

### 3.1 统一工作台

工作台是用户创建、定义、初始化、控制和交互任务的统一界面，其定义固定如下：[BIZ-01][BIZ-02][BIZ-04]

| 固定项 | 定义 | 依据 |
| --- | --- | --- |
| 任务模型 | 一个任务对应一个 Git 代码仓库中的一个分支；该分支是任务成果的主要存储空间。 | [BIZ-02] |
| 任务与作业 | Kunora 管理的 Git 仓库分支对象统一称为“任务”；任务下交给 dsh 执行的单元统一称为“作业”，使用 `JOB-...` 标识。 | [BIZ-06] |
| 任务关联 | 用户把一个代码仓库分支关联到工作台后，工作台创建对应任务记录，立即将其显示在任务列表中，并自动触发任务初始化。 | [BIZ-02][BIZ-04] |
...
| 定义完整度 | 工作台持续显示任务基础信息、六要素、总体方案、路线图及里程碑内容的完善程度，但定义完整度不阻止任务创建或初始化。 | [BIZ-02][BIZ-03][BIZ-04] |
| 自动初始化 | 工作台不提供“启动任务”逻辑。任务创建后自动请求中央服务分配后端节点，由中央服务调用对应 `dshd` 完成任务初始化。 | [BIZ-04] |
| 里程碑定义 | 路线图中的每个里程碑最终都需要具备自己的六要素、总体方案和详细计划，但这些内容不构成任务初始化的前置条件。 | [BIZ-02][BIZ-04] |
| 任务交互 | 任务初始化完成后，工作台提供作业执行进展、实时输出、消息交互、控制和结果查看能力，不再设置第二个人工启动步骤。 | [BIZ-01][BIZ-04][BIZ-06][TECH-02] |
| 初始化失败恢复 | 初始化失败时保留任务关联和已填写内容，工作台显示失败步骤与原因，并提供重试初始化入口；失败期间不展示不可用的任务工作面板页签。 | [BIZ-04][BIZ-06] |
...
| 数据范围 | 持久化任务与后端节点关系、任务相关信息、初始化状态和执行进展，以及节点身份、租约、状态、资源、日志和审计信息。 | [BIZ-01][BIZ-04][TECH-07] |
...
| 初始化编排 | 接受工作台在任务创建后自动提交的初始化请求，完成节点分配和任务—节点关系记录，调用 owner 节点的 `dshd`，并收集初始化状态与结果。 | [BIZ-04] |
...
| 任务初始化 | 接受中央服务对本节点任务发起的初始化请求，完成初始化工作并返回状态与结果；具体动作和完成判定由后续初始化契约定义。 | [BIZ-04] |
| 状态与上报 | 维护 dshd、注册租约、Harness desired/observed state 和 generation；主动向中央服务上报心跳、节点与 Harness 状态、资源摘要、任务初始化和执行进展及日志。 | [BIZ-01][BIZ-04][TECH-03][TECH-05] |
...
依据：`[BIZ-01]` 用户于 2026-08-29 提供的产品说明与确认架构图；`[BIZ-02]` 用户于 2026-08-29 提供的工作台任务模型与定义内容；`[BIZ-03]` 用户于 2026-08-29 补充的任务基础信息要求；`[BIZ-04]` 用户于 2026-08-29 确认废止人工启动并在任务关联后自动初始化；`[BIZ-06]` 用户于 2026-08-29 将 dsh 执行信息页签更名为“作业”，并确认初始化阶段不显示选项卡导航；`[TECH-01]` [分布式管理 MVP 基线](../mvp-baseline.md)；`[TECH-02]` [Harness API 暴露审计](../dsh/harness-api-exposure-audit.md)；`[TECH-03]` [后端节点 HLD](../backend-node-hld.md)；`[TECH-04]` [dshd 服务设计](../dshd-service-design.md)；`[TECH-05]` [中央服务—dshd 接口规范](../interfaces/central-dshd-interface-spec.md)；`[TECH-06]` [Harness 版本冻结基线](../dsh/harness-version-baseline.md)；`[TECH-07]` [中央服务定义基线](../central-service/central-service-hld.md)。
